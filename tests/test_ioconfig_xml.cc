...
TEST_CASE("[IOConfig_XML]: basic initialization with global config", "[IOConfig_XML]")
{
    ...
    SECTION("Sensor parameters from config file")
    {
        ...
        REQUIRE(sensor->permissions == nullptr );
        ...
        REQUIRE(sensor2->defaultPermissions == AccessRW);
        REQUIRE( sensor2->permissions != nullptr );

        auto pit2 = sensor2->permissions->find(500); // Process1
        REQUIRE( pit2 != sensor2->permissions->end() );
        auto& p2 = pit2->second;
        REQUIRE(p2 == AccessRO);
    }
}
