...
const std::list<uniset::ObjectId>& uniset::IDList::ref() const noexcept
{
    return lst;
}
...
// ------------------------------------------------------------------------------------------
bool uniset::check_filter_re( UniXML::iterator& it, const std::string& f_prop, const std::regex& re ) noexcept
{
    if( f_prop.empty() )
        return true;
