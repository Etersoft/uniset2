...
namespace uniset
{
    ...
    {
        aclConfigName = "ACLConfig";
        aclConfigSectionName = "ACLConfig";
    }
    // --------------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------------
    void IOConfig_XML::setAclConfig( const std::string& name, const std::string& section )
    {
        aclConfigName = name;
    ...
    uniset::ACLInfoMap IOConfig_XML::readACLInfo( const std::shared_ptr<Configuration>& _conf, const std::shared_ptr<UniXML>& uxml )
    {
        ...
        auto root = uxml->findNode( uxml->getFirstNode(), "sensors");
        if( !root )
            return out;
        ...
        for( ; it; it++ )
        {
            ...
            if( sname.empty() )
            {
                uwarn << "(IOConfig_XML::readACLInfo): Unknown sensor name... skipped..." << endl;
                continue;
            }
                                      ...
                                      std::shared_ptr<IOController::USensorInfo>& inf ) const
    {
        ...
        auto aclName = it.getProp("acl");
        if( !aclName.empty() )
        {
            auto acl = acls.find(aclName);
            if( acl == acls.end() )
            {
                ostringstream err;
