...
namespace uniset
{
                                      ...
                                      std::shared_ptr<IOController::USensorInfo>& inf ) const
    {
        ...
        auto perm = it.getProp("permission");

        if( !perm.empty() )
        {
            auto m = uniset::AccessMask::fromString(perm);

            if( m != AccessUnknown )
                inf->defaultPermissions = m;
        }

        read_permissions(node, inf);
    ...
    // ------------------------------------------------------------------------------------------
    void IOConfig_XML::read_permissions( xmlNode* node, std::shared_ptr<IOController::USensorInfo>& inf ) const
    {
        UniXML::iterator sit(node);
        UniXML::iterator it(node);

        if( !it.goChildren() )
            return;

        if( !it.find("access",false) )
            return;

        if( !it.goChildren() )
            return;

        inf->permissions = make_shared<IOController::USensorInfo::PermissionsMap>();
        auto conf = uniset_conf();

        for( ; it; it++ )
        {
            auto id = conf->getAnyID(it.getProp("name"));

            if( id == DefaultObjectId )
            {
                ostringstream err;
                err << "(IOConfig_XML::read_permissions): sensor='"
                    << sit.getProp("name") << "' err: "
                    << " Unknown ID for "  << it.getProp("name");
                ucrit << err.str() << endl;
                throw SystemError(err.str());
            }

            const auto p = it.getProp("permission");
            if( !p.empty() )
            {
                auto m = AccessMask::fromString(p);
                if( m == AccessUnknown )
                {
                    ostringstream err;
                    err << "(IOConfig_XML::read_permissions): sensor='"
                ...
                inf->permissions->emplace(id, m);
            }
        }

        if( inf->permissions->empty() )
            inf->permissions = nullptr;
    }
    // ------------------------------------------------------------------------------------------
