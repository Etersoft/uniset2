...
void IOController::activateInit()
{
    for( auto&& io : ioList )
    {
        try
        {
            ...
            if( s->defaultPermissions == AccessNone )
                s->defaultPermissions = defaultAccessMask;
...
long IOController::localGetValue( std::shared_ptr<USensorInfo>& usi, const uniset::ObjectId consumer_id )
{
    if( usi )
    {
        if( !usi->checkMask(consumer_id, defaultAccessMask).canRead() )
        {
            ostringstream err;
...
void IOController::localFreezeValue( std::shared_ptr<USensorInfo>& usi,
                                     ...
                                     uniset::ObjectId sup_id )
{
    if( !usi->checkMask(sup_id, defaultAccessMask).canWrite() )
    {
        ostringstream err;
...
uniset::Message::Priority IOController::getPriority( const uniset::ObjectId sid, const uniset::ObjectId consumer_id )
{
    ...
    if( it != ioList.end() )
    {
        if( !it->second->checkMask(consumer_id, defaultAccessMask).canRead() )
        {
            ostringstream err;
                              ...
                              uniset::ObjectId sup_id )
{
    ...
    if( !it->second->checkMask(sup_id, defaultAccessMask).canWrite() )
    {
        ostringstream err;
...
IOController_i::ShortIOInfo IOController::getTimeChange( uniset::ObjectId sid, const uniset::ObjectId consumer_id )
{
    ...
    if( ait != ioList.end() )
    {
        if( !ait->second->checkMask(consumer_id, defaultAccessMask).canRead() )
        {
            ostringstream err;
...
// -----------------------------------------------------------------------------
uniset::AccessMask IOController::USensorInfo::checkMask( uniset::ObjectId sup, uniset::AccessMask& defaultMask ) const
{
    auto result = AccessUnknown;

    if( permissions )
    {
        auto it = permissions->find(sup);
        if( it != permissions->end() )
            result = it->second;
    }

    if( result == AccessUnknown && defaultPermissions != AccessUnknown )
