...
void Configuration::initParameters()
{
	...
	for( ; it.getCurrent(); it.goNext() )
	{
		...
		else if( name == "LocalTimerService" )
		{
			...
			{
				ostringstream msg;
				msg << "Configuration: TimerService  '" << secTime << "' ?? ?????? ????????????? Ÿ ObjectsMap!!!";
		...
		else if( name == "LocalInfoServer" )
		{
			...
			{
				ostringstream msg;
				msg << "Configuration: InfoServer '" << secInfo << "' ?? ?????? ????????????? Ÿ ObjectsMap!!!";
				unideb[Debug::CRIT] << msg.str() << endl;
				throw Exception(msg.str());
			}
		}
		else if( name == "LocalDBServer" )
		{
			...
			{
				ostringstream msg;
				msg << "Configuration: DBServer '" << secDB << "' ?? ?????? ????????????? Ÿ ObjectsMap!!!";
				unideb[Debug::CRIT] << msg.str() << endl;
				throw Exception(msg.str());
			}
		}
...
//		unideb[Debug::INFO] << "Configuration(createNodesList): ?????? ???? " << nodename << endl;
		...
		{
			unideb[Debug::CRIT] << "Configuration(createNodesList): node '" << nodename << "' ?? ?????? ????????????? Ÿ ObjectsMap!!!" << endl;
			throw Exception("Configuration(createNodesList): node "+nodename+" ?? ?????? ????????????? Ÿ ObjectsMap!!!");
//			continue;
		}
		...
		else
		{
			...
			{
				unideb[Debug::CRIT] << "Configuration(createNodesList): InfoServre '" << iname << "' ?? ?????? ????????????? Ÿ ObjectsMap!!!" << endl;
				throw Exception("Configuration(createNodesList: InfoServer "+iname+" ?? ?????? ????????????? Ÿ ObjectsMap!!!");
			}
		}
	...
	if( unideb.debugging(Debug::INFO) )
		unideb[Debug::INFO] << "Configuration(createNodesList): Ÿ???? ????Ÿ Ÿ ?????? " << lnodes.size() << endl;	
}
// -------------------------------------------------------------------------
...
xmlNode* Configuration::initDebug( DebugStream& deb, const string& _debname )
{
	if( _debname.empty() )
	{
		deb << "(Configuration)(initDebug): INIT DEBUG FAILED!!! ?? ?????? ???Ÿ???? ????\n";
		return 0;
	}
