...
namespace uniset
{
    ...
    Далее приведены основные параметры:
     ...
     - \b --xxx-init-mbval-changed [0|1] или \b init_mbval_changed [0|1] - Глобальный параметр для инициализации флага изменения значения датчика регистра.
     Значения по умолчанию для всех регистров в которых не задан тег init_mbval_changed. Только для регистров на запись с посылкой по изменению.
    ...
    Значение параметра равное 65535 означает, что регистры на запись пишутся только по изменению значения соответствующего
    датчика(значение mbval для регистра).
    - \b tcp_mbmask     - "битовая маска"(uint16_t). Позволяет задать маску для значения. Действует как на значения читаемые из
     регистров, так и записываемые. При этом разрешается привязывать разные датчики к одному и тому же modbus-регистру указывая
    ...
    \warning Регистр должен быть уникальным. И может повторятся только если указан параметр \a nbit или \a mbmask. Для регистров с параметром \a nbyte
    в данной реализации отключена возможность привязывать несколько датчиков к одному регистру т.е. на данный момент \a nbyte соответствует
    только одному регистру.

    - \b tcp_init_mbval_changed [0|1] - Инициализации флага изменения значения для регистров на запись с отправкой по изменению.
    Параметр только для регистров на запись. Можно выставить значение как для конкретного датчика, так и значение по умолчанию
    глобальной настройкой в секции обмена или параметром командной строки.
    ...
    но отличие в том, что при этом режиме ведётся полноценый обмен (и read и write),
     ...
     - \b emSkipExchange - отключить обмен (при этом данные "из SM" обновляются).
