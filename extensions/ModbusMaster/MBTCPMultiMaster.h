...
namespace uniset
{
    ...
    (например для "tcp_mbreg" должен быть задан --xxx-set-prop-prefix tcp_ ).
      ...
      К основным параметрам относятся следующие (префикс \b tcp_ - для примера):
       ...
       - \b tcp_nbit  - номер бита в слове. Используется для DI,DO в случае когда для опроса используется функция читающая слова (03)
    ...
    Т.е. фактически будет опрошен/записан регистр "mbreg+mboffset".
       - \b tcp_pollfactor - [0...n] Частота опроса. n задаёт "частоту" опроса. т.е. опрос каждые 1...n циклов  (зависит от общего polltime)
    ...
    т.к. фактически состояние канала будет определяться только по связи с каким-то одним конкретным устройством.
    ...
    Но плюсом данного способа проверки связи ("modbus-запросом") является то, что соединение поддерживается
    ...
    */
    ...
    {
        public:
            MBTCPMultiMaster( uniset::ObjectId objId, xmlNode* cnode,
