...
```bash
# Конвертация
uniset2-st2js program.st -m mapping.yaml -o output.js

# Запуск на UniSet
uniset2-jscript --js-file output.js --confile configure.xml --js-name MyJSProcess --js-confnode JSProxy
```
...
| `.TcDUT`, `.TcGVL` | TwinCAT XML | Типы данных и global variable lists TwinCAT |
...
**Результат** (`uniset2-st2js program.st -m mapping.yaml`):
```javascript
...
**Результат** (`uniset2-st2js thermostat.st -m thermostat_mapping.yaml`):
```javascript
...
**Пример результата** (`uniset2-st2js program.st -m mapping_plc1.yaml`):
...
```
uniset2-st2js input.st --load-head helpers/utils.js --load-on-start project_sim.js
```
...
```
uniset2-st2js <input...> [-m <mapping.yaml>] [-o <output.js>] [--lib types.yaml] [--ignore-undefined] [--strict] [--struct-flatten] [--programs names] [--main name] [--load-head path] [--load-on-start path] [--debug] [--version]
```
...
| `--load-head` | Добавить `load("PATH")` в начало сгенерированного файла (можно указать несколько раз) |
| `--load-on-start` | Добавить `load("PATH")` в `uniset_on_start()` (можно указать несколько раз) |
| `--debug` | Генерировать метаданные визуализатора и запуск `uniset2-debug.js` |
...
```bash
uniset2-st2js project.xml --ignore-undefined --lib owen_types.yaml -o output.js
```
...
При запуске с `--debug` генерируется расширенная мета-информация, а
сгенерированный JS загружает `uniset2-debug.js` и запускает HTTP-сервер
с веб-интерфейсом отладки на порту 8088. Интерфейс открывается на `/debug/ui`.

Подробное описание runtime API, режимов использования и ограничений:
[../../DEBUG.md](../../DEBUG.md).
...
| `/debug/history?var=X&depth=N` | GET | История значений переменной за окно |
...
### Подключение в сгенерированном JS

```javascript
// st2js добавляет это автоматически при --debug
try {
...
Скрипт запускает pytest-набор из каталога `tests/`.
