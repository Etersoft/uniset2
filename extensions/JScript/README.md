...
Если хочется, чтобы переменная в коде отличалась от реального названия датчика, можно указать
реальное название через поле `sensor: "real_sensor_name"` — тогда `name` будет использован
...
> В качестве `sensor` можно использовать **идентификатор** или **имя** датчика.
...
- `uniset2-iec61131-codesys.js` — функциональные блоки Codesys Util (BLINK, PID, HYSTERESIS, ...)
...
Возможности: live-мониторинг переменных и FB, схема программы,
...
дополнительно доступны метаданные программы для схемы и улучшенного отображения.
...
| Параметр | Описание |
|----------|----------|
| `--js-name <name>` | Имя UniSet-объекта процесса |
| `--js-confnode <name>` | Имя XML-узла с настройками JSProxy (по умолчанию равно `--js-name`) |
| `--js-file <file>` | Запускаемый JS-скрипт |
| `--js-logfile <file>` | Файл для сохранения логов |
| `--js-sleep-msec <msec>` | Пауза между вызовами uniset_on_step (по умолчанию 150 мс) |
| `--js-loopCount <num>` | Количество JS-событий, обрабатываемых за один цикл (по умолчанию 5) |
