...
#define JSProxy_H_
// --------------------------------------------------------------------------
/*! \page pageJScript (JScript) Поддержка скриптов на JS
    ...
    \endcode
    Если хочется чтобы переменная в коде отличалась от реального названия датчика, то можно вписать реальное
    название датчика (в конфиге) при помощи поля \b sensor: "real_sensor_name", тогда name будет использован
    в коде, а привязка будет осуществлена по названию указаному в sensor.
