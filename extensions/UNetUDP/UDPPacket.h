...
namespace uniset
{
    ...
    namespace UniSetUDP
    {
        /*! С учётом того, что ID могут идти не подряд. Сделан следующий формат:
            ...
            - Узел который принимает данные, декодирует их, если нём порядок байт не совпадает.
            Т.е. если все узлы будут иметь одинаковый порядок байт, фактического перекодирования не будет.
        */

        const uint32_t UNETUDP_MAGICNUM = 3; // версия протокола

        struct UDPHeader
        {
            UDPHeader() noexcept;
            uint8_t _be_order: 1; // 1 - BE byte order, 0 - LE byte order
            uint8_t _version: 7; // version 7 bit
            size_t num; // порядковый номер сообщения
            int64_t nodeID;
            int64_t procID;
            uint16_t dcount; /*!< количество булевых величин */
            uint16_t acount; /*!< количество аналоговых величин */
