 ...
 * КЭШ (ДОПОЛНЕНИЕ)
 * ===
 * Вместо 2ух кэшей (аналогово и дискретного) теперь 2 unordered_map этих кэшей
 * Каждый кэш в map добавляется когда приходит пакет с новым UDPMessage::getIdentity() и в дальнейшим используется для этого пакета.
