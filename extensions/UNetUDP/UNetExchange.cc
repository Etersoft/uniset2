    ...
    for( ; n_it.getCurrent(); n_it.goNext() )
    {
        ...
        if( n == conf->getLocalNodeName() )
        {
            ...
            if( !s_sendmode_id.empty() )
            {
                ...
                if( sendmode_id == uniset::DefaultObjectId )
                {
                    ostringstream err;
        ...
        uniset::ObjectId recvmode_id = uniset::DefaultObjectId;

        if( !s_recvmode_id.empty() )
        {
            recvmode_id = conf->getSensorID(s_recvmode_id);

            if( recvmode_id == uniset::DefaultObjectId )
            {
                ostringstream err;
        ...
        const std::string& prefix )
{
    ...
    for( ; n_it.getCurrent(); n_it.goNext() )
    {
        ...
        uniset::ObjectId sendmode_id = uniset::DefaultObjectId;
